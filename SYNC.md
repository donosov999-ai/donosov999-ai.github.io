> # ⚠️ УСТАРЕЛО (21.06.2026): сайт переехал на Astro.
> Этот `deploy.sh` + jsdom-сборка — **LEGACY**. **НЕ деплоить отсюда** — затрёшь живой Astro-сайт (клоббер).
> Новый источник сайта: `/Users/denisonosov/dev/psygames-astro/` · Полный инструктаж: `psygames-astro/HANDOFF_app-chat.md`.
> Этот каталог теперь = только **GitHub Pages зеркало** (rsync собранный Astro `dist/` сюда → `git push`).
> Всё ниже — историческое описание СТАРОГО пайплайна, не применять для правок контента.
